...
style = ttk.Style()
bar_debug_canvas = None
minigame_debug_canvas = None
...
class DebugRenderer:
    def __init__(self, window):
        ...
        self.enabled = True
        self.tag = "debug"
        self.generation = 0   # 🔑 frame / fish ID
    ...
    def clear(self):
        ...
        self.window.after(0, lambda: (
            gen == self.generation and self.canvas.delete(self.tag)
        ))

    def _safe(self, draw_fn):
        gen = self.generation
        def wrapped():
            if gen != self.generation or not self.enabled or not self.canvas:
                return
            draw_fn()
    ...
    def box(self, x, y, w, h, color="red", width=2):
        if not self.enabled or not self.canvas:
            return
        self._safe(lambda:
...
def ensure_minigame_visible():
    if not minigame_window or not minigame_window.winfo_exists():
        show_minigame_window()
    elif not minigame_window.winfo_viewable():
...
def clear_debug_boxes():
    if minigame_canvas:
        minigame_canvas.delete("debug")
...
def draw_box(x, y, w, h, color):
    if not minigame_canvas or not minigame_window:
        return
...
def screen_x_to_overlay(x):
    ox = minigame_window.winfo_rootx()
    return x - ox
...
def stop_clicked(event=None):
    global macro_running, overlay, minigame_window, restart_pending
    global prev_error, last_time
...
def change_bar_width(event=None):
    # Stop current macro and restart
    stop_clicked()
    time.sleep(0.1)
    start_clicked()
...
def restart_macro():
    global macro_running, restart_pending
    global prev_error, last_time
    ...
    def do_restart():
        ...
        # 🔑 invalidate all old debug draws
        debug.next_generation()

        ensure_minigame_visible()
        launch()

    window.after(150, do_restart)
...
def launch(event=None):
    ...
    while macro_running:
        ...
        fish_pos = pixel_search_image(img, 570, 860, (67, 75, 91), 20)
        if fish_pos is None:
            mouse.release(MouseButton.left)
            debug.next_generation()  # 🔑 kill old draw queue
            restart_macro()
            return
        fx = screen_x_to_overlay(fish_pos[0])

        debug.line(fx, 0, fx, 120, color="blue") # Fish position line
        left_edge, right_edge = get_bar_edges_image(img) # Get bar edges

        # --- Primary control ---
        if left_edge is not None and right_edge is not None:
            bx = screen_x_to_overlay(left_edge)
            bw = right_edge - left_edge

            debug.box(bx, 45, bw, 30, color="red")
            bw = right_edge - left_edge
        ...
        # --- Fallback control ---
            ...
            arrow = pixel_search_image(img, 570, 860, (135, 133, 132), 20)
            if arrow:
                ax = screen_x_to_overlay(arrow[0] - 6)
                debug.box(ax, 50, 12, 12, color="yellow")
...
def on_press(key):
    try:
        ...
        elif key == Key.f6: # reload
            change_bar_width()

        elif key == Key.f7: # exit
            stop_clicked()

        elif key == Key.f8: # Debug minigame window
