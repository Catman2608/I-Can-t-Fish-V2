...
def macro_loop(event=None):
    ...
    while macro_running:
        ...
        # Capture region and find fish
        img = capture_region(570, 860, 1350, 910)
        fish_pos = pixel_search_image(img, 570, 860, (67, 75, 91), 0)
        
        # Handle fish not found
        if fish_pos is None:
            fish_miss_count += 1
            set_mouse(False)  # Release mouse to avoid stuck input
        ...
        # Get bar edges and calculate metrics
        left_edge, right_edge = get_bar_edges_image(img)
        ...
        if left_edge is not None and right_edge is not None:
            ...
            # Calculate bar center and bounds
            ...
            # Determine whether to hold based on position
            if fish_pos[0] <= max_left:
                want_hold = False
            elif fish_pos[0] >= max_right:
                want_hold = True
            else:
                error = fish_pos[0] - bar_center
                want_hold = pid_control(error) > 0
        ...
        # Action 5 and 6: Simple Arrow Follow
            ...
            if arrow:
                ...
                want_hold = distance < -6

        # --- Draw fish and arrow visualization ---
        if arrow and arrow_right:
