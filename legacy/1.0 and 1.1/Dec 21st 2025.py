...
def show_overlay_simple():
    ...
    overlay.geometry(f"240x160+20+{screen_height - 220}")
    ...
    # Tooltip rows (persistent)
    global tooltip_labels
    tooltip_labels.clear()

    for row in (4, 5):  # rows reserved for tooltips
        lbl = Label(
            overlay,
            text="",
            fg="white",
...
def save_settings(name):
    ...
    create_label(tab1, f"Saved config: {name}", 0, 1, fg="#00ff00", font=("Segoe UI", 9, "bold"), pady=(10, 5))
...
def load_settings(name):
    ...
    if not os.path.exists(path):
        create_label(tab1, "Config not found", 0, 1, fg="#00ff00", font=("Segoe UI", 9, "bold"), pady=(10, 5))
        return
    ...
    create_label(tab1, f"Loaded Config: {name}", 0, 1, fg="#00ff00", font=("Segoe UI", 9, "bold"), pady=(10, 5))
...
def parse_bbbgggrrr(color_str):
    """
    Converts 'BBB-GGG-RRR' (or 'BBBGGGRRR') into (R, G, B)
    """
    try:
        s = color_str.replace("-", "").strip()
        if len(s) != 9:
            raise ValueError
...
def force_game_focus():
    ...
    """
        try:
            ...
            Label(
                overlay,
                text="Could not focus the window behind: {e}",
                fg="white",
                bg="black",
                font=("Segoe UI", 10)
            ).grid(row=1, column=0)
...
def macro_loop(event=None):
    ...
    bar_rgb = parse_bbbgggrrr(bar_color_entry.get())
    rightbar_rgb = parse_bbbgggrrr(rightbar_color_entry.get())
    ...
    try:
        ...
        while macro_running and not fish_detected and attempts < 400:
            force_game_focus()
            ...
            stable = 0
            while stable < 8:
                if pixel_search(570, 860, 1350, 910, (fish_b, fish_g, fish_r), 5):
                    stable += 1
                    time.sleep(0.005)
                else:
                    break
        ...
        while macro_running:
            ...
            clear_minigame()
            img = capture_region(520, 860, 1400, 910)
            fish_pos = pixel_search_image(img, 570, 860, (fish_b, fish_g, fish_r), 5)
            if fish_pos is None:
                fish_miss_count += 1
            ...
            arrow = None
            arrow_right = None
            arrow = pixel_search_image(img, 570, 860, (arrow_b, arrow_g, arrow_r), 15)
        ...
        set_mouse(False)
        macro_running = False
        create_label(tab1, f"Macro Crashed: {e}", 0, 1, fg="#00ff00", font=("Segoe UI", 9, "bold"), pady=(10, 5)) 
...
create_label(tab1, "Macro Status: Idle", 0, 1, fg="#00ff00", font=("Segoe UI", 9, "bold"), pady=(10, 5))
