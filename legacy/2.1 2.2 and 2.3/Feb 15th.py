...
class App(CTk):
    ...
    def _image_search(self, template_path, confidence=0.85):
        """
        Search for template image inside selected bar area.

        Args:
            template_path (str): Path to template image
            confidence (float): Matching threshold (0.0 - 1.0)

        Returns:
            (center_x, center_y) if found
            None if not found
        """
        ...
        frame = self._grab_screen_region(left, top, right, bottom)
        if frame is None:
            return None
        ...
        return None
    def find_loaded_template(self, confidence=0.85):
        ...
        result = cv2.matchTemplate(gray_frame, self.bar_template, cv2.TM_CCOEFF_NORMED)
        _, max_val, _, max_loc = cv2.minMaxLoc(result)

        if max_val >= confidence:
            h, w = self.bar_template.shape

            return (
                self.bar_left + max_loc[0] + w // 2,
                self.bar_top + max_loc[1] + h // 2
            )
