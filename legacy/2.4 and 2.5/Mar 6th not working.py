...
class App(CTk):
    ...
    def build_misc_tab(self, parent):
        ...
        use_arrow_pid_var = StringVar(value="off")
        self.vars["use_arrow_pid"] = use_arrow_pid_var
        use_arrow_pid_cb = CTkCheckBox(arrow_settings, text="Use PID for Arrows", 
                                           variable=use_arrow_pid_var, onvalue="on", 
                                           offvalue="off")
        use_arrow_pid_cb.grid(row=1, column=0, padx=12, pady=8, sticky="w")
    ...
    def _enter_minigame(self):
        ...
        pid_clamp = float(self.vars["pid_clamp"].get() or 100)
        arrow_pid = self.vars["use_arrow_pid"].get()
        mouse_down = False
        ...
        while self.macro_running: # Main macro loop
            ...
            arrow_center = arrow_center[0] if arrow_center else None
            # ---- Gift box ----
            ...
            # ---- CLEAR MINIGAME ----
            self.clear_overlay()
            # ---- CHECK NORMAL BAR DETECTION ----
            bars_found = (
                left_x is not None and
                right_x is not None and
                fish_x is not None and
                left_x <= fish_x <= right_x
            )

            bar_color = "green"

            # ---- IF BAR NOT FOUND → ESTIMATE FROM ARROW ----
            if not bars_found and arrow_center is not None:
                left_x, right_x = self._update_arrow_box_estimation(
                    last_bar_left,
                    last_bar_right,
                    arrow_center,
                    mouse_down
