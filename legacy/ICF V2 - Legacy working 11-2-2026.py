    ...
    def _enter_minigame(self):
        ...
        while self.macro_running: # Main macro loop
            ...
            if bars_found and bar_center is not None:
                ...
                now = time.time()
                raw_arrow_x = self._find_arrow_indicator_x(img, arrow_hex, arrow_tol, mouse_down)

                arrow_indicator_x = None

                if raw_arrow_x is not None:
                    if self.last_arrow_x is not None and self.last_arrow_time is not None:
                        dt = now - self.last_arrow_time
                        if dt > 0:
                            self.arrow_velocity = (raw_arrow_x - self.last_arrow_x) / dt

                    self.last_arrow_x = raw_arrow_x
                ...
                if arrow_indicator_x is None:
                    pid_found = 1
                    bar_center = None
                elif arrow_indicator_x is not None:
                    ...
                    distance = abs(fish_x - arrow_indicator_x) # Fallback logic
                    if estimated_bar_center is not None:
                        ...
                        # If fish is far, arrow logic is unreliable → fallback
                        if distance > bar_size * 0.7 if bar_size else 120:
                            pid_found = 1
                            bar_center = None
                else:
                    if self.vars["fish_overlay"].get() == "on":
