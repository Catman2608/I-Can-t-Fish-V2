...
class App(CTk):
    def __init__(self):
        ...
        self.last_config = "default.json"
        
        # Track all checkboxes for saving/loading
        self.checkboxes = {}
        self.comboboxes = {}
        ...
        button_frame.grid(row=2, column=0, columnspan=6, pady=10, padx=20, sticky="w")
        
        CTkButton(button_frame, text="Save Settings", command=lambda: self.save_settings(self.last_config)).grid(
            row=0, column=0, padx=5, pady=5
        )
        CTkButton(button_frame, text="Load Settings", command=lambda: self.load_settings(self.last_config)).grid(
            row=0, column=1, padx=5, pady=5
        )
        CTkButton(button_frame, text="Save As...", command=self.save_as_dialog).grid(
            row=0, column=2, padx=5, pady=5
        )
        CTkButton(button_frame, text="Load From...", command=self.load_as_dialog).grid(
        ...
        self.after(100, self.load_last_settings)
        
    def load_last_settings(self):
        """Load the last used settings on startup"""
        if os.path.exists("last_config.txt"):
            try:
                with open("last_config.txt", "r") as f:
                    last_config = f.read().strip()
                    if last_config and os.path.exists(os.path.join("configs", last_config)):
                        self.last_config = last_config
                        self.load_settings(last_config)
                        self.set_status(f"Auto-loaded: {last_config}")
                        return
            except:
                pass
        
        # Load default if exists
        if os.path.exists(os.path.join("configs", "default.json")):
            self.load_settings("default.json")
            self.set_status("Auto-loaded: default.json")
    ...
    def save_as_dialog(self):
        """Open dialog to save settings with custom name"""
        dialog = CTkInputDialog(text="Enter config name:", title="Save Config")
    ...
    def load_as_dialog(self):
        """Open dialog to load settings"""
        configs = self.load_configs()
        if len(configs) > 0 and configs[0] != "No configs found":
            dialog = CTkInputDialog(text="Enter config name:", title="Load Config")
            config_name = dialog.get_input()
            if config_name:
                if not config_name.endswith(".json"):
                    config_name += ".json"
                self.load_settings(config_name)
    ...
    def load_settings(self, config_name):
        ...
        try:
            ...
            self.set_status(f"Error loading config: {str(e)}")
